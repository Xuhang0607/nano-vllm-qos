...
> 历史结果范围：本文 8 请求性能对照和 9 题质量探针属于探索性实验。
> 旧质量对照的保留预算不一致。新版统一保留预算、扩大样本的说明见
> [扩大评测协议](validation_protocol_zh.md)。零匹配页现按近期优先补足预算。

## 新增同预算质量对照

两组独立种子合计 432 题，覆盖直接检索、问题改写、过期信息干扰和两段关联检索。
两种压缩策略均保留 5 页，累计 KV Token 丢弃比例均为 58.3%。

| 策略 | 正确题数 | 准确率 |
| --- | ---: | ---: |
| 不压缩 | 320/432 | 74.1% |
| Sink + Recent | 117/432 | 27.1% |
| Query-Aware | 305/432 | 70.6% |

同预算下 Query-Aware 比固定窗口高约 43.5 个百分点，但比不压缩低约 3.5 个百分点。
问题改写类由不压缩的 31/108 降至 14/108，是后续改进的明确薄弱项。
逐题对照发现 Query-Aware 共丢失 27 个基线正确答案，同时有 12 题由错误变为正确；
采样存在随机性，不能把每个变化都归因于压缩。

这些是单模型、两组数据种子的合成质量结果，不代表通用质量无损，也不代表显存峰值
下降 58.3%。原始数据与分项结果见[独立样本验证](final_validation_zh.md)。
